...
async function whisperRun(audioPath, opts = {}) {
  ...
  if (!cmd) {
    throw new Error(
      ...
      '  pip install -U mlx-whisper        # Apple Silicon (then set BTCH_WHISPER_CMD=mlx_whisper)\n' +
      '  pip install -U openai-whisper\n' +
      '  pip install -U whisper-ctranslate2\n' +
  ...
  // mlx-whisper uses dash-style flags (--output-dir); openai-whisper and
  // whisper-ctranslate2 use underscore-style (--output_dir).
  const isMlx = /mlx[_-]?whisper/i.test(path.basename(cmd));
  const F = (dash, under) => (isMlx ? dash : under);

  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), 'btch-whisper-'));
  let model = opts.model || process.env.BTCH_WHISPER_MODEL || 'base';
  // mlx-whisper expects HuggingFace model paths, e.g. `mlx-community/whisper-base-mlx`.
  // Map common short names. Pass-through anything that already has a slash.
  if (isMlx && !model.includes('/')) {
    const MLX_MAP = {
    ...
    model = MLX_MAP[model] || `mlx-community/whisper-${model}-mlx`;
  }

  const args = [
    audioPath,
    F('--output-format', '--output_format'), 'json',
    F('--output-dir', '--output_dir'), outDir,
    '--model', model,
  ];
  if (opts.language)       args.push('--language', opts.language);
  if (opts.task)           args.push('--task', opts.task);
  if (opts.device && !isMlx) args.push('--device', opts.device);
  if (opts.initial_prompt) args.push(F('--initial-prompt', '--initial_prompt'), opts.initial_prompt);
...
async function transcribeFile(videoOrAudioPath, opts = {}) {
  ...
  if (!alreadyAudio) {
    ...
    // Use a dot-free, bracket-free basename. mlx_whisper derives its output
    // filename from the input stem and breaks badly when the stem contains
    // multiple dots (e.g. `result.data.links[0]…` → output `result.data.json`).
    audioPath = opts.audioOut || path.join(tmpDir, `audio-${Date.now()}.mp3`);
